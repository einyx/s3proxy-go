; S3Proxy Windows Installer Script
; Generated by GoReleaser

!define APPNAME "S3Proxy"
!define COMPANYNAME "S3Proxy"
...
Section "install"
    ...
    FileWrite $0 "echo Installing S3Proxy as Windows service...$\r$\n"
    FileWrite $0 "sc create S3Proxy binPath= `"$INSTDIR\s3proxy.exe -config $INSTDIR\config\config.yaml`" DisplayName= `"S3Proxy Service`" start= auto$\r$\n"
    FileWrite $0 "if %errorlevel% equ 0 ($\r$\n"
    FileWrite $0 "    echo Service installed successfully$\r$\n"
    FileWrite $0 "    echo Use 'sc start S3Proxy' to start the service$\r$\n"
    FileWrite $0 ") else ($\r$\n"
    ...
    FileWrite $0 "@echo off$\r$\n"
    FileWrite $0 "echo Stopping and removing S3Proxy service...$\r$\n"
    FileWrite $0 "sc stop S3Proxy$\r$\n"
    FileWrite $0 "sc delete S3Proxy$\r$\n"
    ...
    CreateShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\s3proxy.exe" "" ""
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${APPNAME}" "DisplayIcon" "$\"$INSTDIR\s3proxy.exe$\""
...
Section "uninstall"
    ...
    ExecWait 'sc stop S3Proxy'
    ExecWait 'sc delete S3Proxy'
